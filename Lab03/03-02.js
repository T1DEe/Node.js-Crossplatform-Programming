...
const server = http.createServer(function (request, response) {
    ...
    else if (url.parse(request.url).pathname === '/03-04') {
        let page = fs.readFileSync('./03-04.html');
        response.writeHead(200, {'Content-Type' : 'text/html; charset=utf-8'})
        response.end(page)
    }
    else if (url.parse(request.url).pathname === '/03-05') {
        let page = fs.readFileSync('./03-05.html');
        response.writeHead(200, {'Content-Type' : 'text/html; charset=utf-8'})
        response.end(page)
    }
    else {
